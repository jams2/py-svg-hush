...
fn get_data_uri_filter(
    keep_data_uri_mime_types: Option<HashMap<String, Vec<String>>>,
) -> impl Fn(&DataUrl) -> DataUrlFilterResult {
    move |data_uri: &DataUrl| -> DataUrlFilterResult {
        match &keep_data_uri_mime_types {
            Some(mapping) => {
                let mime_type = data_uri.mime_type();
                let subtype = mime_type.subtype.as_str();
                if let Some(subtypes) = mapping.get(&mime_type.to_string()) {
...
#[pyfunction]
#[pyo3(
    signature = (svg, /, keep_data_uri_mime_types=None),
    text_signature="(svg, /, keep_data_uri_mime_types=None)"
)]
fn filter_svg(
    py: Python,
    svg: &[u8],
    ...
    filter.set_data_url_filter(get_data_uri_filter(keep_data_uri_mime_types));
